...
x_euler, y_euler = functions.euler(steps_amount)
x_exact, y_exact = functions.exact(steps_amount)
x_euler_improved, y_euler_improved = functions.euler_improved(steps_amount)
x_runge_kutta, y_runge_kutta = functions.runge_kutta(steps_amount)

# local error calculating
euler_err, euler_improved_err, runge_kutta_err = [0.0], [0.0], [0.0]
for i in range(steps_amount):
    euler_err.append(fabs(y_exact[i] - y_euler[i]))
    euler_improved_err.append(fabs(y_exact[i] - y_euler_improved[i]))
    runge_kutta_err.append(fabs(y_exact[i] - y_runge_kutta[i]))
...
for i in range(start,finish):
    ...
    x_euler, y_euler = functions.euler(i)
    x_exact, y_exact = functions.exact(i)
    x_euler_improved, y_euler_improved = functions.euler_improved(i)
    x_runge_kutta, y_runge_kutta = functions.runge_kutta(i)
    # calculating global error
    euler_max_err, euler_improved_max_err, runge_kutta_max_err = 0,0,0
    for j in range(i):
        if fabs(y_exact[j] - y_euler[j]) > euler_max_err:
            euler_max_err = fabs(y_exact[j] - y_euler[j])
        if fabs(y_exact[j] - y_euler_improved[j]) > euler_improved_max_err:
            euler_improved_max_err = fabs(y_exact[j] - y_euler_improved[j])
        if fabs(y_exact[j] - y_runge_kutta[j]) > runge_kutta_max_err:
            runge_kutta_max_err = fabs(y_exact[j] - y_runge_kutta[j])
